'use client';

import { STEPS } from '@/features/onboarding/constants';
import { Step } from './step';
import { StepperBackground } from './stepper-background';

type Props = {
  currentStep: number;
};

const Stepper = ({ currentStep }: Props) => {
  return (
    <div className="fixed top-0 left-0 w-full h-43 flex md:w-auto md:h-full md:p-4 md:relative md:min-w-59.5 lg:min-w-76.5">
      <div className="size-full relative overflow-hidden md:rounded-[10px]">
        <StepperBackground className="absolute w-full h-auto -bottom-[40%] left-0 md:w-auto md:h-full md:top-0" />
        <div className="w-full relative flex justify-center mt-8 z-200 gap-x-4 md:gap-y-8 md:mt-0 md:flex-col md:py-10 md:px-6.75 lg:px-8">
          {STEPS.map(({ stepperTitle }, index) => (
            <Step
              key={index}
              number={index + 1}
              title={stepperTitle}
              selected={currentStep === index + 1}
            />
          ))}
        </div>
      </div>
    </div>
  );
};

export { Stepper };
